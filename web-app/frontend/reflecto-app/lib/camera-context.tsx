...
export const CameraProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Safety Net: If we are not on an active camera page, the camera tracks are immediately stopped (GDPR Compliance)
  useEffect(() => {
    const isCameraPage = pathname === "/camera" || pathname === "/miror" || pathname === "/demo";
    if (!isCameraPage) {
      stopCamera();
    }
  }, [pathname]);
