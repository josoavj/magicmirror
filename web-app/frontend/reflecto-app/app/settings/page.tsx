              ...
              >
                <GlassCard className="flex flex-col md:flex-row items-center gap-6 md:gap-8 p-5 md:p-8">
                  ...
                  <div className="flex-1 text-center md:text-left">
                      <h3 className="text-xl md:text-2xl font-bold text-white mb-1">{profile?.first_name || user?.email?.split('@')[0] || "User"}</h3>
                      <p className="text-slate mb-4">
                         {profile?.age ? `${profile.age} yrs | ` : ""}{profile?.body_type || "H-Shape"} | {profile?.skin_tone || "Warm Tone"}
                      </p>
              ...
              <form className="space-y-5" onSubmit={async (e) => {
                ...
                const formData = new FormData(e.currentTarget);
                  if (user?.id) {
                    await updateProfile(user.id, {
                      ...
                      weight_kg: parseInt(formData.get("weight_kg") as string) || null,
                      body_type: formData.get("morphology") as string,
                      skin_tone: formData.get("skinTone") as string,
                    });
                  }
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    disabled
                    title="Readonly"
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white/50 cursor-not-allowed outline-none transition-all text-sm"
                  />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate uppercase tracking-wider">First Name</label>
                    <input 
                      name="first_name"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate uppercase tracking-wider">Age</label>
                    <input 
                      name="age"
                      type="number"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate uppercase tracking-wider">Height (cm)</label>
                    <input 
                      name="height_cm"
                      type="number"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate uppercase tracking-wider">Weight (kg)</label>
                    <input 
                      name="weight_kg"
                      type="number"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <select 
                      name="morphology"
                      defaultValue={profile?.body_type || "H-Shape"}
                    ...
                    >
                      <option value="H-Shape" className="text-black">H-Shape</option>
                      <option value="V-Shape" className="text-black">V-Shape</option>
                      <option value="A-Shape" className="text-black">A-Shape</option>
                      <option value="X-Shape" className="text-black">X-Shape</option>
                    </select>
                  </div>
                  <div className="space-y-2">
                    ...
                    <select 
                      name="skinTone"
                      defaultValue={profile?.skin_tone || "Warm Tone"}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-gold outline-none transition-all text-sm appearance-none"
                    >
                      <option value="Warm Tone" className="text-black">Warm Tone</option>
                      <option value="Cool Tone" className="text-black">Cool Tone</option>
                      <option value="Neutral Tone" className="text-black">Neutral Tone</option>
                    </select>
                  </div>
                </div>
