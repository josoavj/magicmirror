...
bool _matchesTokens(String value, List<String> targets) {
  ...
  return targets.any(
    (t) =>
        normalizedValue.contains(t.toLowerCase().trim()) ||
        t.toLowerCase().trim().contains(normalizedValue),
  );
}
...
final suggestedOutfitsProvider = Provider<List<OutfitSuggestion>>((ref) {
  final currentMorphology = ref.watch(stableMorphologyProvider);
  ...
  final llmScoreMap = llmScoreMapAsync.maybeWhen(
    data: (d) => d,
    orElse: () => const <String, double>{},
  );
  ...
  var filteredList = allOutfits.where((outfit) {
    ...
    bool matchMorphology = outfit.matchingBodyTypes.any(
      (bt) =>
    ...
    if (userProfile.gender.isNotEmpty) {
      ...
      bool matchGender =
          ...
          gender.contains('binaire') ||
          gender.contains('autre');
    ...
    if (userProfile.preferredStyles.isNotEmpty && outfit.styles.isNotEmpty) {
      bool matchStyle = _hasIntersection(
        outfit.styles,
    ...
    if (weather != null) {
      // Logique de temperature tres basique.
